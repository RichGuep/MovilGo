...
def asignar_grupos_aleatorio(df_cable):
    ...
    while len(masters) >= 2 and len(tecnicos_a) >= 7 and len(tecnicos_b) >= 3:
        ...
        for _ in range(3): 
            ...
            grupos_finales.append(p)
        num_grupo += 1
    for s in (masters + tecnicos_a + tecnicos_b): 
        s['Grupo'] = "Reserva"
        grupos_finales.append(s)
    ...
    if st.button("🚀 Generar Programación Segura"):
        ...
        for fecha in lista_fechas:
            ...
            for g in activos:
                ...
                # REGLA ORO 1: Blindaje T3 -> T1/T2 (Mismo día prohibido)
            ...
            # 3. Motor de Cobertura Obligatoria
            ...
            for g in grupos_nombres:
                ...
                for i in range(1, len(h)):
                    ...
                    if h[i]['Noches_Acum'] > 7:
                        alertas.append(f"⚠️ {g} Exceso Noches")

            if not alertas: st.success("¡Operación Segura!")
            else: st.error(f"Alertas: {', '.join(set(alertas))}")
