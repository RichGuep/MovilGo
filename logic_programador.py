...
def style_malla(df_pivot):
    """Aplica colores y formato visual a las tablas de edición."""
...
# =========================================================
# 3. GESTIÓN DE PERSONAL (ASIGNACIÓN)
# =========================================================
...
def pantalla_personal():
    st.subheader("👥 Planta de Personal y Grupos")
    ...
    if 'df_pers' in st.session_state:
        if st.button("🎲 Asignar Grupos Técnicos y Abordaje"):
...
# =========================================================
# 4. MOTORES DE MALLA (CON COMPENSATORIOS L-V)
# =========================================================
def generar_malla_tecnicos(inicio, fin, desc_conf):
    filas = []
    deudas = {g: 0 for g in GRUPOS_TEC}
    for fecha in pd.date_range(inicio, fin):
        dia_n = DIAS_ES[fecha.weekday()]; sem = fecha.isocalendar()[1]
        asig = {}
        for g, d_pref in desc_conf.items():
            if dia_n == d_pref: asig[g] = "DESCANSO"
        if 0 <= fecha.weekday() <= 4:
            con_deuda = [g for g, v in deudas.items() if v > 0 and g not in asig]
            if con_deuda:
                g_p = sorted(con_deuda, key=lambda x: deudas[x], reverse=True)[0]
                asig[g_p] = "COMPENSADO"; deudas[g_p] -= 1
        ...
        for g in GRUPOS_TEC: 
            if dia_n in desc_conf.values() and asig.get(g) in ["T1","T2","T3"]: deudas[g] += 0 # Lógica de deuda si aplica
...
def generar_malla_abordaje_individual(inicio, fin, d_ba, d_bb):
    ...
    for fecha in pd.date_range(inicio, fin):
        ...
        if 0 <= fecha.weekday() <= 4:
            ...
            for p in p_deuda:
                if len(asig) < (len(personal) - 20): asig[p] = "COMPENSADO"; deudas_p[p] -= 1
        for p in bloque_a:
...
# =========================================================
# 5. INTERFAZ: PARAMETRIZADORES, EDITOR Y AUDITORÍA
# =========================================================
def pantalla_programador():
    tipo = st.sidebar.radio("Módulo", ["Técnicos", "Abordaje"])
    
    # 1. PARAMETRIZADOR DE HORARIOS
    with st.expander("⏰ Parametrizador de Horarios (Inicio/Fin)"):
        config_h = {}
        turnos_list = ["T1", "T2", "T3", "RELEVO", "T1 APOYO", "DISPONIBLE"]
        def_h = {"T1":[time(6,0),time(14,0)], "T2":[time(14,0),time(22,0)], "T3":[time(22,0),time(6,0)], "RELEVO":[time(8,0),time(16,0)], "T1 APOYO":[time(7,0),time(15,0)], "DISPONIBLE":[time(8,0),time(16,0)]}
        cols = st.columns(3)
        for i, t in enumerate(turnos_list):
            with cols[i%3]:
                ini = st.time_input(f"Inicio {t}", def_h[t][0], key=f"ini{t}")
                fin = st.time_input(f"Fin {t}", def_h[t][1], key=f"fin{t}")
                config_h[t] = {"Inicio": ini.strftime("%H:%M"), "Fin": fin.strftime("%H:%M")}
        config_h["DESCANSO"] = config_h["COMPENSADO"] = {"Inicio": "OFF", "Fin": "OFF"}

    # 2. PARAMETRIZADOR DE DESCANSOS
    with st.expander("📅 Configuración de Días de Descanso"):
        if tipo == "Técnicos":
            desc_conf = {}
            cols_d = st.columns(4)
            for i, g in enumerate(GRUPOS_TEC):
                desc_conf[g] = cols_d[i].selectbox(f"{g}", DIAS_ES, index=(i+5)%7, key=f"d{g}")
        else:
            c_ab1, c_ab2 = st.columns(2)
            d_ba = c_ab1.selectbox("Descanso Bloque A", DIAS_ES, index=5)
            d_bb = c_ab2.selectbox("Descanso Bloque B", DIAS_ES, index=6)
    ...
    if m_key in st.session_state:
        ...
        st.subheader("📊 Auditoría de Cobertura (Cupos 10 T1 / 10 T2)")
