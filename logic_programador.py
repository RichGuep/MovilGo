...
# =========================================================
# 3. MOTORES DE GENERACIÓN (LÓGICA DE EQUIDAD)
# =========================================================
...
def generar_malla_abordaje_individual(inicio, fin, d_ba, d_bb):
    ...
    personal = df_emp[df_emp['GrupoAsignado'] == "Abordaje"]['Nombre'].tolist() if not df_emp.empty else [f"Asesor {i}" for i in range(1, 24)]
    
    filas = []
    deudas_p = {p: 0 for p in personal}
    descansos_ley_realizados = {p: 0 for p in personal} # Para asegurar mínimo 2 de ley
    ...
    for fecha in pd.date_range(inicio, fin):
        ...
        # 2. Asignación teórica de descansos de Ley
        for p in bloque_a:
            if dia_n == d_hoy_a and p not in asig: asig[p] = "DESCANSO"
        for p in bloque_b:
            if dia_n == d_hoy_b and p not in asig: asig[p] = "DESCANSO"

        # 3. LLAMADO DE EMERGENCIA EQUITATIVO (Si faltan para cubrir 10/10)
        libres = [p for p in personal if p not in asig]
        if len(libres) < 20:
            faltantes = 20 - len(libres)
            # Ordenamos por quien menos debe y quien ha tenido más descansos reales
            candidatos = sorted([p for p in personal if asig.get(p) == "DESCANSO"], 
                                key=lambda x: (deudas_p[x], -descansos_ley_realizados[x]))
            
            for i in range(min(faltantes, len(candidatos))):
                p_extra = candidatos[i]
                # Solo llamamos si el trabajador ha tenido al menos algún descanso previo o la necesidad es crítica
                del asig[p_extra]
                libres.append(p_extra)
                deudas_p[p_extra] += 1
        
        # Registro de quién sí pudo descansar hoy (Descanso de Ley)
        for p in personal:
            if asig.get(p) == "DESCANSO": descansos_ley_realizados[p] += 1
...
def pantalla_programador():
    st.title("📅 Programador de Malla MovilGO")
    tipo = st.sidebar.radio("Módulo Operativo", ["Técnicos", "Abordaje"])
    
    with st.expander("⏰ Horarios Configurados"):
        t_list = ["T1", "T2", "T3", "RELEVO", "T1 APOYO", "DISPONIBLE"]
        cols = st.columns(3)
        for i, t in enumerate(t_list):
            cols[i%3].text(f"{t}: Configurado")

    with st.expander("📅 Configuración de Descansos"):
        if tipo == "Técnicos":
    ...
    if m_key in st.session_state:
        ...
        st.subheader(f"📝 Editor de Malla: {tipo}")
        ...
        st.subheader("📊 Auditoría de Cobertura")
        if "T1" in conteo.columns and "T2" in conteo.columns:
            ...
            if (conteo["T1"] < 10).any() or (conteo["T2"] < 10).any():
                st.error("🚨 Atención: Hay días con menos de 10 personas en T1 o T2.")
        
        st.subheader("⚖️ Reporte de Equidad (Turnos por Persona)")
