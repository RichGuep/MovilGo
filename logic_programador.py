...
# --- 1. CONFIGURACIÓN DE ESTILOS (PANDAS 2.x READY) ---
def aplicar_estilos_malla(styler):
    """Aplica el código de colores oficial usando .map() para compatibilidad."""
    colores = {
...
def guardar_malla_github(df):
    ...
    try:
        contents = repo.get_contents("malla_historica.xlsx")
        repo.update_file("malla_historica.xlsx", "Malla V7.0 - Cobertura Garantizada", output.getvalue(), contents.sha)
...
def auditar_malla_visual(df):
    ...
    dias_sin_cob = [f for f in cobertura.index if any(cobertura.get(t, pd.Series(0)).loc[f] == 0 for t in ["T1", "T2", "T3"])]
    ...
    if 'Deuda_Compensatorio' in df.columns:
        m3.metric("Deuda Global", int(df.groupby('Grupo')['Deuda_Compensatorio'].last().sum()))
...
def pantalla_programador():
    ...
    if st.button("🚀 Generar Malla Inteligente"):
        ...
        for fecha in lista_fechas:
            ...
            # Compensación Inmediata: Pagar deuda si no es su descanso fijo
            for g in grupos_n:
            ...
            # Garantizar Cobertura: Mínimo 3 grupos para T1, T2, T3
            ...
            for g in grupos_n:
                resultados.append({
                    "Grupo": g, "Fecha_Col": col_name, "Turno": turnos_hoy[g], "Fecha_Raw": fecha_dt, 
