...
def guardar_github(df, nombre_archivo):
    ...
    if not repo: 
        st.error("No se pudo conectar a GitHub. Verifica el Token en Secrets.")
        return
    buffer = io.BytesIO()
...
# =========================================================
# 3. ESTILOS Y AUDITORÍA PROFESIONAL
# =========================================================
...
def ejecutar_auditoria(df, tipo):
    ...
    # Conteos por turno para validación de cobertura
    ...
    if tipo == "Técnicos":
        ...
        # Auditoría Abordaje: Estricto 10 pers en T1 y 10 en T2 (Bloques de 5)
        for f in t1.index:
    ...
    equidad["Días_Trabajados"] = equidad[["T1", "T2", "T3"]].sum(axis=1)
    
    return errores, cobertura, equidad

# =========================================================
# 4. GENERACIÓN TÉCNICOS (LEY + DEUDAS + EQUIDAD)
# =========================================================
def generar_malla_tecnicos(inicio, fin, descansos_ley):
    ...
    for fecha in pd.date_range(inicio, fin):
        ...
        # Rotación de Equidad (Para no saturar Grupo 1 y 2 con T3)
...
# =========================================================
# 5. GENERACIÓN ABORDAJE (ROTACIÓN EN BLOQUE SÓLIDO)
# =========================================================
def generar_malla_abordaje(inicio, fin, desc_cfg, ciclo):
    ...
    for fecha in pd.date_range(inicio, fin):
        ...
        asig_bloque = {}
        # 10 pers T1 (2 gps) y 10 pers T2 (2 gps)
        ...
        for g in GRUPOS_ABO:
            turno_del_grupo = asig_bloque.get(g, "RELEVO" if g == gp_relevo else "DESCANSO")
            for p in PERSONAL_ABO[g]:
                filas.append({
                    "Fecha": fecha, "Sujeto": p, 
                    "Turno": turno_del_grupo if g not in gps_descansan else "DESCANSO"
                })
    return pd.DataFrame(filas)

# =========================================================
# 6. PANTALLA PRINCIPAL (PROGRAMADOR)
# =========================================================
def pantalla_programador():
    st.sidebar.markdown("---")
    tipo = st.sidebar.radio("🔧 Módulo Operativo", ["Técnicos", "Abordaje"])
    
    st.subheader(f"📅 Planificador Maestro - {tipo}")
    ...
    with st.container(border=True):
        ...
        ciclo = "Fijo"
        if tipo == "Abordaje":
            ciclo = st.selectbox("Rotación de Bloques", ["Diario", "Quincenal", "Mensual"], index=1)
    ...
    if key in st.session_state:
        ...
        # Auditoría tras cada cambio en el editor
        ...
        st.divider()
        st.subheader("📊 Análisis de Equidad y Carga")
        ...
        with f2:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                equidad.to_excel(writer, sheet_name='Metricas')
